...
char output[50]="";// output which contains postfix expression
...
void main() 
{
	...
	int i;
	char equ[50];
	printf("Welcome to the program to convert infix expression to postfix expression\n\n");
	...
	for (i=0; i<=strlen(equ); i++)
	{
		if (isalpha(equ[i]))   
		{
			for (j = strlen(output); j <= (strlen(output) + 1); j++)
			{
				output[j] += equ[i]; // operands are added in output
				break;
			}
		}
		else
	...
	printf("Postfix expressin : %s", output);	
}
